...
namespace YiJian.CardReader.CardReaders.Wante
{
    ...
    /// 万特 读卡器
    /// </summary>
    public class WanteReader : AbstractCardReader
    {
        ...
        public WanteReader(string driverRelativePath) : base(driverRelativePath)
        {
        }

        public override void CloseDevice()
        {
            if (WanteDriver.iClosePort() != 0)
            {
                // TODO: 关闭端口失败
            }
        }
        ...
        {
            if (WanteDriver.iOpenPort(driverMessage) != 0)
            {
                _log.Error("万特读卡器 打开设备端口失败：{Error}", driverMessage.ToString());
                return false;
            }
            WanteDriver.iPosBeep();
            return true;
        }
        ...
        public override IDCardInfo? ReadIdCard()
        {
            ...
            cardInfo.Clear();
            WanteDriver.iPosBeep();
            var ret = WanteDriver.iReaderIDCard_CS(pBmpFile, cardInfo, b64Info, driverMessage);
            if (ret != 0)
            {
                _log.Error("万特读卡器读取卡片信息错误，返回结果：{Ret}  错误原因：{Msg}",ret,driverMessage);
        ...
        public override MedicareCardInfo? ReadMedicareCard()
        {
            cardInfo.Clear();
            WanteDriver.iPosBeep();
            var ret = WanteDriver.iReadSicard_CS(0x11, cardInfo, driverMessage);
            if (ret != 0)
            {
                _log.Error("万特读卡器读取卡片信息错误，返回结果：{Ret}  错误原因：{Msg}", ret, driverMessage);
                throw new InvalidOperationException(driverMessage.ToString());
            ...
            else
            {
                ...
                if (!string.IsNullOrWhiteSpace(cardInfo.ToString()))
                {
                    ...
                    var medCardInfo = new MedicareCardInfo(
                        ...
                        cardInfoSplit[3],
                        cardInfoSplit[4],
                        Utils.SetDateTime(cardInfoSplit[5]));
                    return medCardInfo;
                }
                _log.Error("万特读卡器 读卡失败, 内容为空");
            }
        ...
        public override RFIDCardInfo? ReadRfCard()
        {
            ...
            WanteDriver.iPosBeep();
            var ret = WanteDriver.MifareOnCardReadHEX_CS(addr, keyMode, key, cardInfo, driverMessage);
            if (ret != 0)
            {
                _log.Error("万特读卡器读取卡片信息错误，返回结果：{Ret}  错误原因：{Msg}", ret, driverMessage);
                throw new InvalidOperationException(driverMessage.ToString());
        ...
        {
            WanteDriver.iPosBeep();
            var ret = WanteDriver.iReadMagCard(10, 3, cardInfo, driverMessage);
            if (ret != 0)
            {
                _log.Error("万特读卡器读取卡片信息错误，返回结果：{Ret}  错误原因：{Msg}", ret, driverMessage);
                throw new InvalidOperationException(driverMessage.ToString());
