...
namespace YiJian.MasterData.MasterData
{
    ...
    /// LabTreeAppService
    /// </summary>
    public class LabTreeAppService : MasterDataAppService // BaseTreeAppService<ExamTree>,
    {
        private ILabTreeRepository _treeRepository;
        private readonly IMemoryCache _cache;
        private const string Lab_TreeList_Key = "lab:tree:list:all";
        /// <summary>
        /// 所有的树的Base方法
        /// </summary>
        public LabTreeAppService(ILabTreeRepository repo, IMemoryCache cache)
        {
        ...
        /// <returns></returns>
        public async Task<LabTree> InsertNodeAsync(AddTreeDto dto)
        {
            var entity = new LabTree()
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<LabTree> DeleteNodeAsync(DbPrimaryType id)
        {
        ...
        /// <returns></returns>
        public async Task<LabTree> ChangeParentAsync(DbPrimaryType id, NullableDbPrimaryType parentid)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<LabTree> GetByIdAsync(DbPrimaryType id)
        {
        ...
        /// <returns></returns>
        public async Task<IList<LabTree>> ListNodesByFullPathAsync(string parentFullPath)
        {
        ...
        /// <returns></returns>
        public async Task<IList<LabTree>> ListAllNodesByIdAsync(DbPrimaryType id)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<LabTree> GetTreeByRootIdAsync(string id)
        {
        ...
        /// <returns></returns>
        public async Task<LabTree> GetTreeByRootPathAsync(string name)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<IList<LabTree>> ListTreeListByRootIdAsync(string id)
        {
            List<LabTree> treelist = new List<LabTree>
            {
        ...
        /// <returns></returns>
        public async Task<IList<LabTree>> GetTreesByNameAsync(string name, bool useCache = false)
        {
            //查询出Project 有的数据但是 Tree 没有的数据
            var noBind = await _treeRepository.GetNoBindTreeProjectAsync();
            if (useCache)
            {
                var cacheData = _cache.Get<IList<LabTree>>(Lab_TreeList_Key);
                if (cacheData == null)
                {
                    cacheData = await _treeRepository.ListTreesByNameAsync(name);
                    BindProject(cacheData, noBind);
                    _cache.Set(Lab_TreeList_Key, cacheData, TimeSpan.FromHours(CacheKey.EXAMCATALOGTIME));
                }
                return cacheData;
            }
            else
        ...
        private void BindProject(IList<LabTree> examTrees, IList<LabTree> noBindList)
        {
        ...
        /// <returns></returns>
        public async Task<IList<LabTree>> GetChildrenByIdAsync(DbPrimaryType parentId, string KeyWord)
        {
            //根节点标志
            if (parentId == "0")
            {
                ...
                {
                    return new List<LabTree> { rootNode };
                }
        ...
        {
            return _treeRepository.UpdateAsync(updateTree.Id, updateTree.NodeName, updateTree.Sort);
        }
