namespace YiJian.Nursing
{
    ...
    /// <summary>
    /// 表:护理项目表 DataSeedContributor
    /// </summary>
    public class ParaItemDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        public async Task SeedAsync(DataSeedContext context)
        {
			if (await _paraItemRepository.GetCountAsync() > 0)
			{
				return;
			}            
            /*
        ...
        #region constructor
        public ParaItemDataSeedContributor(
            IParaItemRepository paraItemRepository, 
            ParaItemManager paraItemManager,
            IGuidGenerator guidGenerator)
        {
            _paraItemRepository = paraItemRepository;
            _paraItemManager = paraItemManager;
