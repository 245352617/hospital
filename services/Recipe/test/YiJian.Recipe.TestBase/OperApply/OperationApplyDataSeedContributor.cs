namespace YiJian.Recipe
{
    ...
    /// <summary>
    /// 分诊患者id DataSeedContributor
    /// </summary>
    public class OperationApplyDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        public async Task SeedAsync(DataSeedContext context)
        {
			if (await _operationApplyRepository.GetCountAsync() > 0)
			{
				return;
			}            
            /*
			...
			await _operationApplyRepository.InsertAsync(operationApply); 
            */
        }

        #region constructor
        public OperationApplyDataSeedContributor(
            IOperationApplyRepository operationApplyRepository, 
            OperationApplyManager operationApplyManager,
